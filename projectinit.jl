...
using Pkg
Pkg.activate(".")
emptysettingproject = Dict("ProjectPathStructure"=>
                                Dict("extras"=>Dict(), 
								     "docs"=>Dict(), 
									 "src"=>Dict(), 
									 "wdata"=>Dict("structs"=>Dict())),
                           "ProjectAuthor" => 
						        Dict("Alexander Batjanovskii"=>
								     ("AlexandrBatjanovsky", "alexandersn@yandex.ru")))

isinstalled(pkg::String) = any(x -> x.name == pkg && x.is_direct_dep,
                               values(Pkg.dependencies()))
if !isinstalled("JSON"); Pkg.add("JSON"); end
using JSON
